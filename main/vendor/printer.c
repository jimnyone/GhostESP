...
void print_text_to_printer(const char *printer_ip, const char *text,
  ...
  if (sock < 0) {
    ...
    glog("Failed to create printer socket\n");
    return;
  }
  ...
  if (err != 0) {
    ...
    glog("Failed to connect to printer at %s\n", printer_ip);
    close(sock);
    return;
  }

  ESP_LOGI(TAG, "Connected to printer at %s:%d", printer_ip, PRINTER_PORT);
  glog("Connected to printer at %s\n", printer_ip);
...
>>>>>>> 54_nowe
  ...
  glog("Printer connection closed\n");
}
...
void handle_printer_command(int argc, char **argv) {
  ...
  if (printer_ip == NULL || printer_ip[0] == '\0') {
    glog("Error: Printer IP cannot be empty\n");
    return;
  }
  if (text == NULL || text[0] == '\0') {
    glog("Error: Text to print cannot be empty\n");
    return;
  }
  if (font_px <= 0) {
    glog("Error: Invalid font size, using default (50)\n");
    font_px = 50;
  }
  ...
  glog("Printing: \"%s\" (size: %d, alignment: %s)\n", text,
